...
class MessageHandler:
    ...
    async def handle_incoming_message(
        self, message: ClientMessage, ws_session: WebSocketSessionStorage
    ):
    ...
    async def handle_ping_message(
        self, message: PingMessage, ws_session: WebSocketSessionStorage
    ):
        """
        ...
        await ws_session.send_message_callback(
            type=ServerMessageType.PONG, client_message=message
        )
    ...
    async def handle_open_message(
        self, message: OpenMessage, ws_session: WebSocketSessionStorage
    ):
        """
    ...
    async def handle_close_message(
        self, message: CloseMessage, ws_session: WebSocketSessionStorage
    ):
        ...
        if self.speech_provider:
            await self.speech_provider.shutdown_session(
                session_id,
                ws_session,
                finalize_callback=lambda: self.finalize_session(
                    session_id, ws_session, message
                ),
            )

    async def handle_connection_probe(
        self, message: OpenMessage, ws_session: WebSocketSessionStorage
    ):
        """
        Handle connection probe
